...
from .nonlinear import nonlinear_transformation 
...
class flare22_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # # 获取图像数据的最小值和最大值
        # data_min = np.min(data_load_ori["data"])
        # data_max = np.max(data_load_ori["data"])    
        # # print("image range:", data_min,"---", data_max)
        # # before_nonlinear_transformation = data_load_ori["data"].copy()

        # data_load_ori["data"] = nonlinear_transformation(
        #     data_load_ori["data"], minn=data_min, maxx=data_max
        # )


        
        # print("difference after nonlinear_transformation:", np.abs(data_load_ori["data"] - before_nonlinear_transformation).max())
        # data_min = np.min(data_load_ori["data"])
        # data_max = np.max(data_load_ori["data"]) 
        # print("nonlinear image range:", data_min,"---", data_max)
        data_trans_ori = self.transforms(**data_load_ori)
        # print("flare22_dataset transforms -----------------------")
        # 检查转换后是否正常
        # data_infos = {
