# -*- coding: utf-8 -*-
"""
Created: Nov 2018

@author: Jue Jiang

Date modified: July 7 2020
@author: Harini Veeraraghavan
Description of changes: Cleaning up code and documentation

"""

# -*- coding: utf-8 -*-

import torch.nn.functional as F
import torch
from collections import OrderedDict
from torch.autograd import Variable
import itertools
from util import util as util
from util.image_pool import ImagePool
from .base_model import BaseModel
from . import networks
from loss.MarginalSegLoss import Marginal_DC_and_CE_loss
from loss.WGANLoss import calc_gradient_penalty


class Organ_attention(BaseModel):
    def name(self):
        return 'Organ_attention'

    def initialize(self, opt):
        BaseModel.initialize(self, opt)

        nb = opt.batchSize
        size = opt.fineSize
        if opt.isTrain:
            self.input_A = self.Tensor(nb, opt.input_nc, size, size) # input A
            self.input_B = self.Tensor(nb, opt.output_nc, size, size) # input B
            self.input_A_y = self.Tensor(nb, opt.output_nc, size, size) # input B
            self.input_B_y = self.Tensor(nb, opt.output_nc, size, size) # input B
            if opt.partial_label:
                self.input_A_y_partial_label = self.Tensor(nb, opt.organ_num+1)
                self.input_A_y_partial_label = self.input_A_y_partial_label.cuda()
            self.input_A=self.input_A.cuda()
            self.input_B=self.input_B.cuda()
            self.input_A_y=self.input_A_y.cuda()
            self.input_B_y=self.input_B_y.cuda()

        else:
            self.test_A = self.Tensor(nb, opt.output_nc, size, size) # input B
            self.test_B = self.Tensor(nb, opt.output_nc, size, size) # input B
            self.test_A_y = self.Tensor(nb, opt.output_nc, size, size) # input B

            self.test_A = self.test_A.cuda()
            self.test_B = self.test_B.cuda()
            self.test_A_y = self.test_A_y.cuda()

        self.netG_A = networks.define_G(1, 1,
                                        opt.ngf, opt.which_model_netG, opt.norm, not opt.no_dropout, opt.init_type, self.gpu_ids)
        self.netG_B = networks.define_G(1, 1,
                                        opt.ngf, opt.which_model_netG, opt.norm, not opt.no_dropout, opt.init_type, self.gpu_ids)

        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")


        if not self.isTrain:
            use_sigmoid = True
            self.netD_A = networks.define_D(opt.output_nc, opt.ndf,
                                            opt.which_model_netD,
                                            opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, self.gpu_ids)

            self.netD_B = networks.define_D(opt.input_nc, opt.ndf,
                                            opt.which_model_netD,
                                            opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, self.gpu_ids)


        if self.isTrain:
            # use_sigmoid = opt.no_lsgan
            if opt.gan_type == 'lsgan' or opt.gan_type == 'wgan-gp':
                use_sigmoid = False
            else:
                use_sigmoid = True

            self.netD_A = networks.define_D(opt.output_nc, opt.ndf,
                                            opt.which_model_netD,
                                            opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, self.gpu_ids)
            if opt.Organ_Attention_Type == 'concat':
                self.netD_A_local = networks.define_D(2, opt.ndf,
                                                      opt.which_model_netD,
                                                      opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, self.gpu_ids)
            else:    
                self.netD_A_local = networks.define_D(1, opt.ndf,
                                                      opt.which_model_netD,
                                                      opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, self.gpu_ids)
            self.netD_B = networks.define_D(opt.input_nc, opt.ndf,
                                            opt.which_model_netD,
                                            opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, self.gpu_ids)
        organ_numer= opt.organ_num + 1
        #Unet|Split-Unet|DenseNet')
        if opt.Net == 'Unet':
            self.netSeg = networks.get_Unet(1, organ_numer, opt.init_type, self.gpu_ids)
        elif opt.Net == 'DenseNet':
            self.netSeg = networks.get_FCDenseNet_67(1, organ_numer, opt.init_type, self.gpu_ids)
        elif opt.Net == 'Split-Unet':
            self.netSeg = networks.get_Unet_split(1, organ_numer, opt.init_type, self.gpu_ids)
        else:
            raise NotImplementedError()

        self.atten_type=opt.Organ_Attention_Type

        if not self.isTrain or opt.continue_train:
            which_epoch = opt.which_epoch

            if self.isTrain:
                self.load_network(self.netG_A, 'G_A', which_epoch)
                self.load_network(self.netG_B, 'G_B', which_epoch)
                self.load_network(self.netD_A, 'D_A', which_epoch)
                self.load_network(self.netD_B, 'D_B', which_epoch)
                self.load_network(self.netD_A_local, 'D_A_local', which_epoch)
                self.load_network(self.netSeg, 'Seg_B', which_epoch)

        if self.isTrain:
            self.old_lr = opt.lr
            self.fake_A_pool = ImagePool(opt.pool_size)
            self.fake_B_pool = ImagePool(opt.pool_size)
            self.fake_B_local_pool = ImagePool(opt.pool_size)

            # self.criterionGAN = networks.GANLoss(use_lsgan=not opt.no_lsgan, tensor=self.Tensor)
            self.criterionGAN = networks.GANLoss(gan_type=opt.gan_type, tensor=self.Tensor)
            self.criterionCycle = torch.nn.L1Loss()
            self.criterionIdt = torch.nn.L1Loss()
            # initialize optimizers

            self.optimizer_G = torch.optim.Adam(itertools.chain(self.netG_A.parameters(), self.netG_B.parameters()),
                                                lr=opt.lr, betas=(opt.beta1, 0.999), amsgrad=True)
            self.optimizer_D_A = torch.optim.Adam(self.netD_A.parameters(), lr=opt.lr * opt.D_lr_mult, betas=(opt.beta1, 0.999), amsgrad=True)
            self.optimizer_D_A_local = torch.optim.Adam(self.netD_A_local.parameters(), lr=opt.lr * opt.D_lr_mult, betas=(opt.beta1, 0.999), amsgrad=True)
                     
            self.optimizer_D_B = torch.optim.Adam(self.netD_B.parameters(), lr=opt.lr * opt.D_lr_mult, betas=(opt.beta1, 0.999), amsgrad=True)
            
            self.optimizer_Seg = torch.optim.Adam(self.netSeg.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999), amsgrad=True)
            

            self.optimizers = []
            self.schedulers = []
            self.optimizers.append(self.optimizer_G)
            self.optimizers.append(self.optimizer_D_A)
            self.optimizers.append(self.optimizer_D_A_local)         
                               
            self.optimizers.append(self.optimizer_D_B)
            
            self.optimizers.append(self.optimizer_Seg)        
                         
            for optimizer in self.optimizers:
                self.schedulers.append(networks.get_scheduler(optimizer, opt))

            self.seg_loss = Marginal_DC_and_CE_loss(soft_dice_kwargs={})

        print('---------- Networks initialized -------------')
        networks.print_network(self.netG_A)
        networks.print_network(self.netG_B)
        if self.isTrain:
            networks.print_network(self.netD_A)
            networks.print_network(self.netD_B)
            networks.print_network(self.netSeg)
        print('-----------------------------------------------')

    def net_G_A_load_weight(self,weight):
        self.load_network(self.netG_A, 'G_A', weight)

    def net_G_B_load_weight(self,weight):
        self.load_network(self.netG_B, 'G_B', weight)

    def net_D_A_load_weight(self,weight):
        self.load_network(self.netD_A, 'D_A', weight)

    def net_D_B_load_weight(self,weight):
        self.load_network(self.netD_B, 'D_B', weight)

    def net_Seg_B_load_weight(self, epoch):
        self.load_network(self.netSeg, 'Seg_B', epoch)

    def get_curr_lr(self):
        self.cur_lr=self.optimizer_Seg_A.param_groups[0]['lr']

    def load_pre_trained_net(self,weight):
        self.load_network(self.netD_A, 'D_A', weight)
        self.load_network(self.netD_B, 'D_B', weight)
        self.load_network(self.netSeg, 'Seg_B', weight)
        self.load_network(self.netG_A, 'G_A', weight)
        self.load_network(self.netG_B, 'G_B', weight)
        self.load_network(self.netD_A_local, 'D_A_local', weight)

    def set_test_input(self, input):
        if self.opt.G_A:
            input_A11 = input['CT']
            self.test_A.resize_(input_A11.size()).copy_(input_A11)
        if self.opt.Seg:
            input_B11 = input['MR']
            self.test_B.resize_(input_B11.size()).copy_(input_B11)
        if self.opt.G_B:
            input_B11 = input['MR']
            self.test_B.resize_(input_B11.size()).copy_(input_B11) 

    def set_input(self, input):
        AtoB = self.opt.which_direction == 'AtoB'

        input_A1=input['CT']
       
        input_A11,input_A12=torch.split(input_A1, input_A1.size(1)//2, dim=1)  #mt_BN use
        input_A12 = input_A12.long()
        
        input_B1=input['MRI']

        if 'CT_partial_label' in input.keys():
            input_A1_partial_label = input['CT_partial_label']
        else:
            input_A1_partial_label = None
        
        if input_B1.size(1) > 1:
            input_B11,input_B12=torch.split(input_B1, input_B1.size(1)//2, dim=1)  #mt_BN use
            input_B12 = input_B12.long()
        else:
            input_B11, input_B12 = input_B1, None
         
        self.input_A.resize_(input_A11.size()).copy_(input_A11)
        self.input_B.resize_(input_B11.size()).copy_(input_B11)
        

        self.input_A_y.resize_(input_A12.size()).copy_(input_A12)
        if input_B12 is not None:
            self.input_B_y.resize_(input_B12.size()).copy_(input_B12)
        else:
            self.input_B_y = None

        if input_A1_partial_label is not None:
            self.input_A_y_partial_label.resize_(input_A1_partial_label.size()).copy_(input_A1_partial_label)
        else:
            self.input_A_y_partial_label = None

    def cal_segmentation_loss(self, input, target, partial_label=None):

        return self.seg_loss(input, target, partial_label)


        
    def forward(self):
        self.real_A = Variable(self.input_A)
        self.real_A_y=Variable(self.input_A_y)
        self.real_B = Variable(self.input_B)
        if self.input_B_y is not None:
            self.real_B_y = Variable(self.input_B_y)
        else:
            self.real_B_y = None

    def test_forward(self):
        return_results = {}
        if self.opt.G_A:
            real_A = Variable(self.test_A)
            fake_B = self.netG_A(real_A)
            return_results['fake_B'] = fake_B
        if self.opt.G_B:
            real_B = Variable(self.test_B)
            fake_A = self.netG_A(real_B)
            return_results['fake_A'] = fake_A
        if self.opt.Seg:
            real_B = Variable(self.test_B)
            real_B_seg = self.netSeg(real_B)
            if self.opt.organ_num == 1:
                real_B_seg = torch.sigmoid(real_B_seg)
                real_B_seg = real_B_seg[:, 0]
            else:
                real_B_seg = torch.softmax(real_B_seg, dim=1)
                _, real_B_seg = torch.max(real_B_seg, dim=1)
            return_results['real_B_seg'] = real_B_seg
        return return_results

    def get_image_paths(self):
        return self.image_paths

    def cal_seg_loss(self,netSeg,pred,gt, partial_label=None):
        if self.opt.Net == 'Split-Unet':
            self.pred = netSeg.forward_1(pred)
        else:
            self.pred = netSeg(pred)

        lmd = self.opt.SegLambda_B
        seg_loss = lmd * self.cal_segmentation_loss(self.pred, gt, partial_label)

        return seg_loss


    def backward_segmentation_B_stream(self):
        gt_A = self.real_A_y # gt
        img_A = self.real_A # img
        # img_AB = self.netG_A(self.real_A) # gt
        img_AB = self.netG_A(self.real_A).detach() # gt

        if self.opt.Net == 'Split-Unet':
            pred1 = self.netSeg.forward_1(img_AB)
            loss_1 = self.cal_segmentation_loss(pred1, gt_A, self.input_A_y_partial_label)
            pred2 = self.netSeg.forward_2(img_AB)
            loss_2 = self.cal_segmentation_loss(pred2, gt_A, self.input_A_y_partial_label)
            total_loss = loss_1+loss_2
        else:
            pred = self.netSeg(img_AB)
            total_loss = self.cal_segmentation_loss(pred, gt_A, self.input_A_y_partial_label)

        self.seg_loss_Attention_fake_B = total_loss.item() #.data[0]

        #total_loss=seg_loss_AB+seg_loss_Attention_B
        total_loss.backward()
        self.seg_loss_AB_=self.seg_loss_Attention_fake_B#

    def dice_loss_binary(self,input, target):
        smooth = 1.
        iflat = input.view(-1)
        tflat = target.view(-1)
        iflat=iflat.cuda()
        tflat=tflat.cuda()
        intersection = (iflat * tflat).sum()

        return ((2. * intersection + smooth)/(iflat.sum() + tflat.sum() + smooth))

    def backward_D_basic(self, netD, real, fake):
        # Real
        pred_real = netD(real)
        loss_D_real = self.criterionGAN(pred_real, True)
        # Fake
        pred_fake = netD(fake.detach())
        loss_D_fake = self.criterionGAN(pred_fake, False)
        # Combined loss
        loss_D = (loss_D_real + loss_D_fake) * 0.5
        # backward
        # loss_D.backward()
        return loss_D
    
    def backward_D_basic_A(self, netD, real, fake):
        # Real
        pred_real = netD(real)
        loss_D_real = self.criterionGAN(pred_real, True)
        # Fake
        pred_fake = netD(fake.detach())
        loss_D_fake = self.criterionGAN(pred_fake, False)
        # Combined loss
        loss_D = (loss_D_real + loss_D_fake) * 0.5

        # backward
        #loss_D.backward()
        return loss_D

    def backward_D_A(self):
        fake_B = self.fake_B_pool.query(self.fake_B) # same to self.fake_B_local_pool
        
        loss_D_A = self.backward_D_basic_A(self.netD_A, self.real_B, fake_B)
        Total_loss_DA=loss_D_A
        if self.opt.gan_type == 'wgan-gp':
            use_gpu = True if fake_B.device.type=='cuda' else False
            gradient_penalty = calc_gradient_penalty(self.netD_A, self.real_B.data, fake_B.data, use_cuda=use_gpu)
            Total_loss_DA += self.opt.lambda_gp * gradient_penalty
            # print('D_A Discrimination:{:.4f}, gradient penalty:{:.4f}'.format(loss_D_A, gradient_penalty))

        self.loss_D_A = loss_D_A.item()#.data[0]
        self.fake_B_in_D=fake_B.data[0]
        return Total_loss_DA


    def backward_D_basic_A_local(self, netD, real, fake):
        # Real
        pred_real = netD(real)
        loss_D_real = self.criterionGAN(pred_real, True)
        # Fake
        pred_fake = netD(fake.detach())
        loss_D_fake = self.criterionGAN(pred_fake, False)
        # Combined loss
        loss_D = (loss_D_real + loss_D_fake) * 0.5
        # backward
        #loss_D.backward()
        return loss_D        

    def backward_D_A_local(self):
        fake_B = self.fake_B_pool.query(self.fake_B)

        if self.opt.Net == 'Split-Unet':
            att_map_fake_B=self.netSeg.forward_2(fake_B)
        else:
            att_map_fake_B=self.netSeg(fake_B)


        att_map_fake_B=F.softmax(att_map_fake_B, dim=1)

        att_map_fake_B_1=att_map_fake_B[:, 0, :, :]
        att_map_fake_B_struct=1-att_map_fake_B_1

        att_map_fake_B_struct=att_map_fake_B_struct.view(-1, 1, 288, 288)
        # att_map_fake_B_struct=att_map_fake_B_struct.unsqueeze(1)

        if self.atten_type == 'additive':
            fake_B_struct_cat_att=fake_B+att_map_fake_B_struct#torch.cat((fake_B_local,att_map_fake_B),1)
        elif self.atten_type == 'dot':
            fake_B_struct_cat_att=fake_B*att_map_fake_B_struct#torch.cat((fake_B_local,att_map_fake_B),1)
        else:
            fake_B_struct_cat_att=torch.cat((fake_B,att_map_fake_B_struct),1)
       
        if self.opt.Net=='Split-Unet':
            att_map_real_B=self.netSeg.forward_2(self.real_B)
        else:
            att_map_real_B=self.netSeg(self.real_B)


        att_map_real_B=F.softmax(att_map_real_B, dim=1)
        att_map_real_B_1=att_map_real_B[:,0,:,:] 
        att_map_real_B_struct=1-att_map_real_B_1

        att_map_real_B_struct=att_map_real_B_struct.view(-1,1,288, 288)
        # att_map_real_B_struct=att_map_real_B_struct.unsqueeze(1)

        if self.atten_type == 'additive':
            real_B_struct_cat_att=self.real_B+att_map_real_B_struct #torch.cat((fake_B_local,att_map_fake_B),1)
        elif self.atten_type == 'dot':
            real_B_struct_cat_att=self.real_B*att_map_real_B_struct #torch.cat((fake_B_local,att_map_fake_B),1)
        else:
            real_B_struct_cat_att=torch.cat((self.real_B, att_map_real_B_struct), 1)


        loss_D_A_local_struct = self.backward_D_basic_A_local(self.netD_A_local, real_B_struct_cat_att, fake_B_struct_cat_att)

        Total_loss_DA=loss_D_A_local_struct
        if self.opt.gan_type == 'wgan-gp':
            use_gpu = True if fake_B_struct_cat_att.device.type=='cuda' else False
            gradient_penalty = calc_gradient_penalty(self.netD_A_local, real_B_struct_cat_att.data, fake_B_struct_cat_att.data, use_cuda=use_gpu)
            Total_loss_DA += self.opt.lambda_gp * gradient_penalty
            # print('D_A_struct Discrimination:{:.4f}, gradient penalty:{:.4f}'.format(loss_D_A_local_struct, gradient_penalty))

        self.loss_D_A_local_struct=loss_D_A_local_struct.item()
 
        self.att_map_real_B_struct=att_map_real_B_struct.data
        self.att_map_fake_B_struct=att_map_fake_B_struct.data
 
        return Total_loss_DA

    def backward_D_A_global_and_local(self):
        
        DA_global=self.backward_D_A()
        
        DA_local=self.backward_D_A_local()
        D_A_loss_total=DA_global+self.opt.local_D_weight*DA_local
        D_A_loss_total.backward()

    def backward_D_B(self):
        fake_A = self.fake_A_pool.query(self.fake_A)
        loss_D_B = self.backward_D_basic(self.netD_B, self.real_A, fake_A)
        Total_loss_DB = loss_D_B
        if self.opt.gan_type == 'wgan-gp':
            use_gpu = True if fake_A.device.type=='cuda' else False
            gradient_penalty = calc_gradient_penalty(self.netD_B, self.real_A.data, fake_A.data, use_cuda=use_gpu)
            Total_loss_DB += self.opt.lambda_gp * gradient_penalty
            # print('D_B Discrimination:{:.4f}, gradient penalty:{:.4f}'.format(loss_D_B, gradient_penalty))
        Total_loss_DB.backward()
        self.loss_D_B = loss_D_B.item()#.data[0]

    def backward_G(self):
        lambda_idt = self.opt.identity
        lambda_A = self.opt.lambda_A
        lambda_B = self.opt.lambda_B
        # Identity loss
        if lambda_idt > 0:
            # G_A should be identity if real_B is fed.
            idt_A = self.netG_A(self.real_B)
            loss_idt_A = self.criterionIdt(idt_A, self.real_B) * lambda_B * lambda_idt
            # G_B should be identity if real_A is fed.
            idt_B = self.netG_B(self.real_A)
            loss_idt_B = self.criterionIdt(idt_B, self.real_A) * lambda_A * lambda_idt

            self.idt_A = idt_A.data
            self.idt_B = idt_B.data
            self.loss_idt_A = loss_idt_A.item()#.data[0]
            self.loss_idt_B = loss_idt_B.item()#.data[0]
        else:
            loss_idt_A = 0
            loss_idt_B = 0
            self.loss_idt_A = 0
            self.loss_idt_B = 0


        # GAN loss D_A(G_A(A))
        fake_B = self.netG_A(self.real_A)
        pred_fake = self.netD_A(fake_B)
        loss_G_A = self.criterionGAN(pred_fake, True)
        
        # local D
        fake_B_local=fake_B

        if self.opt.Net == 'Split-Unet':
            att_fake_B=self.netSeg.forward_1(fake_B)
        else:
            att_fake_B=self.netSeg(fake_B)

        att_fake_B=F.softmax(att_fake_B, dim=1) # the softmax activation is used to get the probabilistic segmentation
        
        att_fake_B_1=att_fake_B[:, 0, :, :]
        att_map_fake_B=1-att_fake_B_1

        att_map_fake_B=att_map_fake_B.view(att_map_fake_B.size(0), 1, 288, 288) #可以更灵活
        fake_B_local=fake_B_local.view(fake_B_local.size(0), 1, 288, 288)

        if self.opt.Organ_Attention_Type == 'additive':
            fake_B_local_cat_att=fake_B_local+att_map_fake_B
        elif self.opt.Organ_Attention_Type == 'dot':
            fake_B_local_cat_att=fake_B_local*att_map_fake_B
        else:
            fake_B_local_cat_att=torch.cat((fake_B_local,att_map_fake_B),1)

        pred_fake_local = self.netD_A_local(fake_B_local_cat_att)
        # pred_fake_local = self.netD_A_local(fake_B_local_cat_att.detach())

        loss_G_A_local = self.criterionGAN(pred_fake_local, True) # the adv local on G
        

        # GAN loss D_B(G_B(B))
        fake_A = self.netG_B(self.real_B)
        pred_fake = self.netD_B(fake_A)
        loss_G_B = self.criterionGAN(pred_fake, True)

        # Forward cycle loss
        rec_A = self.netG_B(fake_B)
        loss_cycle_A = self.criterionCycle(rec_A, self.real_A) * lambda_A

        # Backward cycle loss
        rec_B = self.netG_A(fake_A)
        loss_cycle_B = self.criterionCycle(rec_B, self.real_B) * lambda_B

        seg_loss_B=self.cal_seg_loss(self.netSeg, fake_B, self.real_A_y, self.input_A_y_partial_label) # the seg_B_loss
        # seg_loss_B=self.cal_seg_loss(self.netSeg, fake_B.detach(), self.real_A_y, self.input_A_y_partial_label) # the seg_B_loss


        # combined loss
        #Here is the adversarial loss for generator including the structure generator
        loss_G = loss_G_A + loss_G_B + loss_cycle_A + loss_cycle_B + loss_idt_A + loss_idt_B+seg_loss_B+self.opt.local_D_weight*loss_G_A_local 
        loss_G.backward()

        if self.opt.gradient_clip:
            torch.nn.utils.clip_grad_norm_(self.netG_A.parameters(), max_norm=self.opt.max_norm)
            torch.nn.utils.clip_grad_norm_(self.netG_B.parameters(), max_norm=self.opt.max_norm)

        fake_B_and_local=torch.cat((fake_B, fake_B_local), 2)

        self.fake_B = fake_B.data
        self.fake_B_and_local=fake_B_and_local.data
        #self.fake_B_variable=Variable(self.fake_B)
        self.fake_A = fake_A.data
        self.rec_A = rec_A.data
        self.rec_B = rec_B.data

        self.loss_G_A = loss_G_A.item()#.data[0]
        self.loss_G_B = loss_G_B.item()#.data[0]
        self.loss_cycle_A = loss_cycle_A.item()#.data[0]
        self.loss_cycle_B = loss_cycle_B.item()#.data[0]
        #self.loss_seg_A=seg_loss_A.item()#.data[0]
        self.loss_seg_B=seg_loss_B.item()#.data[0]
        self.loss_feature=0#feature_loss.data[0]
        self.loss_G_A_loca=loss_G_A_local.item()#.data[0]
        self.seg_loss_Real_B=0#seg_acc_Real_B.data[0]
        self.seg_loss_fake_B=0#seg_acc_fake_B.data[0]
        #self.att_map_fake_B_parotid=att_map_fake_B.view(1,1,256,256).data
        self.fake_B_local=fake_B_local.data
        self.loss_G_A_local=loss_G_A_local.item()#.data[0]


    def optimize_parameters(self, flag):
        
        # forward
        self.forward()

        self.optimizer_G.zero_grad()
        self.backward_G()
        self.optimizer_G.step()

        self.optimizer_D_A.zero_grad()
        self.optimizer_D_A_local.zero_grad()
        # SegB_att + G->D_struct
        if flag:
            self.backward_D_A_global_and_local()
            self.optimizer_D_A.step()
            self.optimizer_D_A_local.step()

            self.optimizer_D_B.zero_grad()
            self.backward_D_B()
            self.optimizer_D_B.step()

            self.optimizer_Seg.zero_grad()
            self.backward_segmentation_B_stream()
            #G->SegB_att, G->SegB     f(segB)~~f(segB_att)
            self.optimizer_Seg.step()
        # print(self.real_A.max(), self.real_B.max(), self.fake_A.min(), self.fake_B.min(), '347')
        # print(self.real_A.min(), self.real_B.min(), self.fake_A.max(), self.fake_B.max(), '347')

    def get_current_errors(self):
        ret_errors = OrderedDict([('D_A', self.loss_D_A), ('G_A', self.loss_G_A), ('Cyc_A', self.loss_cycle_A),
                                 ('D_B', self.loss_D_B), ('G_B', self.loss_G_B), ('Cyc_B',  self.loss_cycle_B),
                                 ('Seg_B',  self.loss_seg_B),('loss_G_A_loca',self.loss_G_A_loca),('seg_loss_Real_B',  self.seg_loss_Real_B),
                                 ('seg_loss_fake_B',self.seg_loss_AB_),('seg_loss_Attention_fake_B',self.seg_loss_Attention_fake_B),
                                 ('G_A_struct',self.loss_G_A_local),
                                 ('D_A_struct',  self.loss_D_A_local_struct)])

        return ret_errors

    def get_current_visuals(self):
        real_A = util.tensor2im(self.input_A)
        fake_B = util.tensor2im(self.fake_B)
        rec_A = util.tensor2im(self.rec_A)
        real_B = util.tensor2im(self.input_B)
        fake_A = util.tensor2im(self.fake_A)
        rec_B = util.tensor2im(self.rec_B)
        real_Ay=util.tensor2im_hd_neck(self.input_A_y)

        if self.opt.Net == 'Split-Unet':
            pred_B=self.netSeg.forward_1(self.real_B)
            
        else:
            pred_B=self.netSeg(self.real_B)

        pred_B=torch.argmax(pred_B, dim=1)
        pred_B=pred_B.view(pred_B.size(0), 1, 288, 288)
        pred_B=pred_B.data
        seg_B=util.tensor2im_hd_neck(pred_B)

        ret_visuals = OrderedDict([('real_A', real_A), ('fake_B', fake_B), ('rec_A', rec_A),
                                   ('real_B', real_B), ('fake_A', fake_A), ('rec_B', rec_B),
                                   ('real_B_seg', seg_B),
                                   ('real_A_GT_seg', real_Ay),
                                   ])
        
        return ret_visuals

    def save(self, label):
        self.save_network(self.netG_A, 'G_A', label, self.gpu_ids)
        self.save_network(self.netD_A, 'D_A', label, self.gpu_ids)
        self.save_network(self.netG_B, 'G_B', label, self.gpu_ids)
        self.save_network(self.netD_B, 'D_B', label, self.gpu_ids)
        self.save_network(self.netSeg, 'Seg_B', label, self.gpu_ids)
        self.save_network(self.netD_A_local, 'D_A_local', label, self.gpu_ids)























        
